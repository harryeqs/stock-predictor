...
from camel.models import SambaModel, ModelFactory
...
"""]
# Uncomment to see more diverse results
# """
...
# """]
model_config=SambaCloudAPIConfig(temperature=0.0, max_tokens=1800)

sambanova_model = ModelFactory.create(
...
def get_worker_agent(msg):
    return ChatAgent(system_message=msg,
                     model=sambanova_model,
                     tools=guess_tools,
                     token_limit=4096)

invigilator = get_worker_agent(invigilator_task)
...
for prompt in prompts:
    i = int(i) + 1
    worker = get_worker_agent(prompt)
